...
const khmerAudioUrl = "/media/khmer-audio-new.mp3";
...
export function playChhing(ctx: AudioContext, dest: AudioNode, time: number, gainLevel: number) {
  try {
    ...
    osc.type = "sine";
    osc.frequency.setValueAtTime(3520, time);
...
function renderStep(
  ...
  if (trackId === "angkor_dawn") {
    ...
    if (step % 8 === 0) playChhing(ctx, dest, time, vol * 0.3);
    const melodyMap: Record<number, number> = {
      0: KHMER_SCALE_D[5], 2: KHMER_SCALE_D[7], 4: KHMER_SCALE_D[8], 6: KHMER_SCALE_D[9],
      8: KHMER_SCALE_D[11], 10: KHMER_SCALE_D[9], 12: KHMER_SCALE_D[8], 14: KHMER_SCALE_D[6],
      16: KHMER_SCALE_D[5], 18: KHMER_SCALE_D[8], 20: KHMER_SCALE_D[9], 22: KHMER_SCALE_D[11],
      24: KHMER_SCALE_D[12], 26: KHMER_SCALE_D[11], 28: KHMER_SCALE_D[9], 30: KHMER_SCALE_D[7],
    };
    if (melodyMap[step]) playRoneatTone(ctx, dest, melodyMap[step], time, 0.9, vol * 0.5);
    ...
    const notes: Record<number, number> = {
      0: KHMER_SCALE_F[5], 2: KHMER_SCALE_F[6], 4: KHMER_SCALE_F[7], 6: KHMER_SCALE_F[8],
      8: KHMER_SCALE_F[9], 10: KHMER_SCALE_F[8], 12: KHMER_SCALE_F[7], 14: KHMER_SCALE_F[6],
      16: KHMER_SCALE_F[7], 18: KHMER_SCALE_F[8], 20: KHMER_SCALE_F[9], 22: KHMER_SCALE_F[8],
      24: KHMER_SCALE_F[7], 26: KHMER_SCALE_F[6], 28: KHMER_SCALE_F[5], 30: KHMER_SCALE_F[4],
    };
    if (notes[step]) playRoneatTone(ctx, dest, notes[step], time, 0.8, vol * 0.45);
  } else if (trackId === "temple_garden") {
    if (step === 0 || step === 12) playKongVongTone(ctx, dest, 130.81, time, 4.2, vol * 0.4);
    if (step % 6 === 0) playChhing(ctx, dest, time, vol * 0.25);
    const notes: Record<number, number> = {
      0: 261.63, 3: 329.63, 6: 392.0, 9: 440.0, 12: 523.25, 15: 440.0, 18: 392.0, 21: 329.63,
    };
    if (notes[step]) playRoneatTone(ctx, dest, notes[step], time, 1.4, vol * 0.4);
    ...
    const notes: Record<number, number> = {
      0: 293.66, 2: 349.23, 4: 392.0, 6: 440.0, 8: 523.25, 10: 440.0, 12: 392.0,
      14: 349.23, 16: 392.0, 18: 440.0, 20: 523.25, 22: 587.33, 24: 523.25, 26: 440.0,
      28: 392.0, 30: 293.66,
    };
    if (notes[step]) playRoneatTone(ctx, dest, notes[step], time, 0.75, vol * 0.45);
  }
}
...
export async function getRenderedTrackBuffer(
  ...
  if (trackBufferCache.has(cacheKey)) return trackBufferCache.get(cacheKey)!;
...
export const BGM_TRACKS: Record<string, BgmTrack> = {
  angkor_dawn: {
    id: "angkor_dawn",
    name: "Angkor Dawn",
    description: "Calm traditional Pinpeat atmosphere with gentle Roneat Ek melody and Kong Vong resonance.",
    bpm: 54,
    totalSteps: 32,
    audioUrl: khmerAudioUrl,
    playStep: (ctx, dest, step, vol) => renderStep("angkor_dawn", ctx, dest, step, ctx.currentTime, vol),
    renderToBuffer: (ctx) => getRenderedTrackBuffer(ctx, BGM_TRACKS.angkor_dawn),
  },
  royal_khmer: {
    id: "royal_khmer",
    name: "Royal Khmer",
    description: "Elegant Khmer palace ensemble with steady Chhing cadence and flowing Kong Touch flourishes.",
    bpm: 60,
    totalSteps: 32,
    playStep: (ctx, dest, step, vol) => renderStep("royal_khmer", ctx, dest, step, ctx.currentTime, vol),
  ...
  temple_garden: {
    ...
    bpm: 46,
    totalSteps: 24,
    playStep: (ctx, dest, step, vol) => renderStep("temple_garden", ctx, dest, step, ctx.currentTime, vol),
    renderToBuffer: (ctx) => getRenderedTrackBuffer(ctx, BGM_TRACKS.temple_garden),
  },
  ouk_chaktrang: {
    id: "ouk_chaktrang",
    name: "Ouk Chaktrang",
    description: "Focused tournament match atmosphere with solemn bronze and rhythmic Pinpeat cadence.",
    bpm: 64,
    totalSteps: 32,
    playStep: (ctx, dest, step, vol) => renderStep("ouk_chaktrang", ctx, dest, step, ctx.currentTime, vol),
